...
template<Int T> inline void readmod(register T& x , register T& Mod) { bool sign = 0; x = 0; while (!((Ch = getchar()) == '-' ? Ch = getchar(), sign = 1 : isdigit(Ch))); while (x = ten(x) + (Ch & 15),x%=Mod, isdigit(Ch = getchar())); x = sign ? opposite(x) : x; }
